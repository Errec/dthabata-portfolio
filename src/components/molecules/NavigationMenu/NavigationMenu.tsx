...
const orientationClassName: Record<NonNullable<NavigationMenuProps["orientation"]>, string> = {
  horizontal: "flex-row items-center gap-[52px]",
    ...
    <ul className={cn("flex", orientationClassName[orientation])}>
      {links.map((link) => (
        <li key={link.href}>
          <Link
            href={link.href}
            className="text-xl font-bold text-muted-foreground transition-colors hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 focus-visible:ring-offset-background"
            onClick={onNavigate}
          >
            {link.label}
          </Link>
